"""
Ingazi (wb-e-learning) integration service.
...
class IngaziService:
    ...
    def __init__(self):
        self.base_url = settings.ingazi_api_url.rstrip("/")

    async def _fetch_paths(self) -> list[dict]:
        """Fetch all paths with their courses from Strapi. Cached for 5 minutes."""
    ...
    def _compute_completion_rate(self, progress_json: dict, paths: list[dict]) -> float:
        ...
        - Count total courses across all paths
        ...
        completed_courses = 0
        for path_id, entries in progress_json.items():
    ...
    def _compute_course_progress(self, progress_json: dict, paths: list[dict]) -> dict:
        ...
        Returns dict keyed by path_id with course-level completion info.
        """
        # Build set of completed course IDs from progressJson
        completed_course_ids = set()
        for path_id, entries in (progress_json or {}).items():
            if not isinstance(entries, list):
                continue
            for entry in entries:
                if isinstance(entry, dict) and entry.get("dataLevel") == "completedCourse":
    ...
    def _count_modules(self, progress_json: dict) -> dict:
        """Count completed courses and paths from progressJson."""
        completed_courses = 0
        completed_paths = 0

        if not progress_json:
            return {"completed_courses": 0, "completed_paths": 0}
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                return {
                    ...
                    "ingazi_completed": modules["completed_paths"],
                    "total_ingazi": len(paths),
    ...
    async def enroll(self, email: str, password: str = "Ingazi2024!") -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return {
                        "ingazi_user_id": str(strapi_user.get("id", "")),
                ...
                if response.status_code == 400:
                    ...
                    if find_resp.status_code == 200:
                        ...
                        if users:
                            ...
                            return {
                                "ingazi_user_id": str(user.get("id", "")),
                                "platform_url": "http://localhost:3000",
                            }
    ...
    async def sync_all_progress(self, db: AsyncSession) -> dict:
        ...
        for ben, user in rows:
            ...
            if email in strapi_progress:
                ...
                if ben.ingazi_completion_rate is None or float(ben.ingazi_completion_rate) != rate:
                    ben.ingazi_completion_rate = rate
                    ben.ingazi_course_progress = course_progress
                    ben.ingazi_last_sync = now
                    updated += 1
                elif ben.ingazi_course_progress != course_progress:
                    ben.ingazi_course_progress = course_progress
                    ben.ingazi_last_sync = now
    ...
    def _mock_progress(self) -> dict:
        ...
        return {
            ...
            "total_modules": 0,
            "url": None,
            "note": "Ingazi platform is currently unreachable",
        }
